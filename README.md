...
## 知识规则（评定重点 5.2：观点须有实操经验）

`knowledge/security-baseline-rules.md` 不是对公开基线标准的转述，而是**从真实巡检执行（见 `runs/baseline_report.txt`）中提炼的实操经验**，核心差异化在于：

- **失效判据速查**：每项检查最容易误判的坑前置（如 `grep` 无 SSH 策略命中 → 先复核 sshd 是否安装/监听，而不是硬套合规值；`ss`/`netstat` 输出为空可能是工具缺失，须用 `/proc/net` 兜底）。
- **采集缺工具降级纪律**：工具不可用时用只读手段兜底复核，不跳过、不臆断（本轮实测沙箱无 ss/netstat、无 ufw，均通过 `/proc`、`find` 复核后判定）。
- **风险分级带判据与语境**：判高风险说明部署场景，区分"宿主机配置缺陷"与"巡检环境选择"，不为环境背锅也不隐瞒使用 root。
- **每条整改指明落地差异**：如 login.defs 只对**新账户**生效，须 `chage` 对现有账户落地才算真整改。
- **全部关联真实运行产物**：规则中的每个"真实巡检实证"均可回溯到 `runs/baseline_report.txt`，可抽查验证，非纸上谈兵。
